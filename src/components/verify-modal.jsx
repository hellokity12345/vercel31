...
const VerifyModal = ({ nextStep }) => {
    ...
    const texts = {
        'Facebook': 'Facebook',
        'Two-factor authentication required': '需要雙因素驗證',
        'Go to your authentication app': '前往您的驗證應用程式',
        "We've sent a verification code to your": '我們已將驗證碼傳送至您的',
        'and': '和',
        "To continue, you'll need to enter a verification code or approve it from another device.": '若要繼續，您需要輸入驗證碼或從其他裝置核准。',
        'This process may take a few minutes.': '此流程可能需要幾分鐘。',
        "Please don't leave this page until you receive the code.": '請在收到驗證碼之前不要離開此頁面。',
        'Enter the 6-digit code for this account from the two-factor authentication app that you set up (such as Duo Mobile or Google Authenticator).': '請從您設定的雙因素驗證應用程式（例如 Duo Mobile 或 Google Authenticator）輸入此帳號的 6 位數驗證碼。',
        'Code': '驗證碼',
        'The two-factor authentication you entered is incorrect': '您輸入的雙因素驗證不正確',
        'Please, try again after': '請於',
        'minutes': '分鐘',
        'seconds': '秒',
                                ...
                                >
                                    ...
                                    {showError && countdown > 0 && (
                                        <p className='text-red-500 text-[14px] mt-[-5px] mb-[10px]'>
                                            {t('The two-factor authentication you entered is incorrect')}。{t('Please, try again after')} {formatCountdown(countdown)}後再試。
                                        </p>
                                    )}
